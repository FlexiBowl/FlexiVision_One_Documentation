(troubleshooting)=
# **Troubleshooting**

Select the component or topic for which you need support.

## **Mechanical Installation**

:::{card} VisionController
:link: troubleshooting_vision_controller
:link-type: ref
:::

:::{card} Camera
:link: troubleshooting_camera
:link-type: ref
:::

:::{card} Toplight
:link: troubleshooting_toplight
:link-type: ref
:::

:::{card} Ambient Light
:link: troubleshooting_luce_ambientale
:link-type: ref
:::

## **Wiring and Connections**

:::{card} Power Supply
:link: troubleshooting_alimentazione
:link-type: ref
:::

:::{card} Ethernet
:link: troubleshooting_ethernet
:link-type: ref
:::

:::{card} Pneumatics
:link: troubleshooting_pneumatica
:link-type: ref
:::

:::{card} Camera Connection
:link: troubleshooting_connessione_camera
:link-type: ref
:::

:::{card} Toplight Connection
:link: troubleshooting_connessione_toplight
:link-type: ref
:::

:::{card} Multi-Device
:link: troubleshooting_multi
:link-type: ref
:::

## **Initial Configuration**

:::{card} FlexiBowl Setup
:link: troubleshooting_fb_setup
:link-type: ref
:::

:::{card} Hopper Setup
:link: troubleshooting_hopper_setup
:link-type: ref
:::

:::{card} Robot Setup
:link: troubleshooting_robot_setup
:link-type: ref
:::

:::{card} Camera Setup
:link: troubleshooting_cam_setup
:link-type: ref
:::

## **Calibration and Recipes**

:::{card} Camera Calibration
:link: troubleshooting_calib_cam
:link-type: ref
:::

:::{card} New Recipe Creation
:link: troubleshooting_nuova_ricetta
:link-type: ref
:::

:::{card} New Model Creation
:link: troubleshooting_nuovo_modello
:link-type: ref
:::

:::{card} Models and ROI
:link: troubleshooting_modelli_roi
:link-type: ref
:::

:::{card} Clearances
:link: troubleshooting_istogrammi
:link-type: ref
:::

:::{card} Robot Pick
:link: troubleshooting_robot_pick
:link-type: ref
:::

## **Advanced Functions and Monitoring**

:::{card} FlexiBowl Wizard
:link: troubleshooting_fb_wizard
:link-type: ref
:::

:::{card} Hopper
:link: troubleshooting_conf_tramoggia
:link-type: ref
:::

```{toctree}
:hidden:
26b_Installazione_Mecc.md
26c_Cablaggio_Connessioni.md
26d_Conf_Iniziale.md
26e_Calib_Cam.md
26g_Creazione_Ricette_Modelli.md
26h_FlexiBowl_Wizard.md
26i_Conf_Tramoggia.md
```

### License cannot be activated

```{note}
**License issues**

If the license does not activate:
- Check the Internet connection
- Verify the Windows system date/time is correct
- Make sure the key has been entered exactly as provided
```

---
